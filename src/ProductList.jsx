...
function ProductList() {
  ...
  const handleAddedToCart = (i)=>{
    const existingItem = cart.find((item) => item.name === i.name);
    if(existingItem){
      return true;
    }
    return false;
        ...
        <div style={styleObjUl}>
          ...
          <div>
            ...
            <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
              <h1 style={{position:"relative"}} className="cart">
                <div style={{position:"absolute", fontSize:"24px", top:21, right:25}}>{totalCartItems()}</div>
                <svg
              ...
              <div className="product-list">
                {category.plants.map((plant, plantIndex) => (
                  <div className="product-card" key={plantIndex}>
                    ...
                    <div className="product-description" style={{color:"grey"}}>{plant.description}</div>
                    <button
                      className="product-button"
                      onClick={() => handleAddToCart(plant)}
                      disabled={handleAddedToCart(plant) ? true : false}
                      style={{backgroundColor:handleAddedToCart(plant)?"grey": "#4CAF50"}}
                    >
                      Add to Cart
                    </button>
                  </div>
